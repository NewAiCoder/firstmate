...
- Round cap, with a severity escape. Rounds 1 to 3 run normally: fix everything the gate selects.
  ...
  The cap counts rounds, not just the pipeline's own auto-loop. When \`auto_fix.review\` is 0 (or otherwise off), the pipeline does not loop reviews for you, and you drive convergence yourself by calling \`no-mistakes axi respond --action fix\` again after each review; every such worker-driven fix response is a round exactly like a pipeline-counted one, tallied on the same run, and the round-3 soft cap and severity escape apply to it identically - keep your own count across those calls rather than assuming an uncounted pipeline is an uncapped one.

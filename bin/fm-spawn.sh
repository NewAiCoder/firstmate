...
launch_template() {
  ...
  case "$harness" in
    ...
    # what the worker's git and PR paths read. It also keeps this in force
    # regardless of which settings scopes end up loaded, since a worker's
    # settings sources are not otherwise guaranteed to load the `user` scope.

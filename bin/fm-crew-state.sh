...
if ! pane_readable "$BACKEND_TARGET"; then
  # A confirmed-gone verdict below is exactly the point at which the
  # per-worker memory limit (systemd MemoryMax scope) becomes relevant: an
  # OOM kill is positive death evidence, so surface it in the same detail
  # string rather than as a separate check.
  GONE_SUFFIX=""
  if [ -n "$MEMORY_SCOPE" ]; then
    MEMORY_RESULT=$(fm_agent_memory_result "$MEMORY_SCOPE") || MEMORY_RESULT=
    if fm_agent_memory_is_oom "$MEMORY_RESULT"; then
      GONE_SUFFIX=" (killed by the per-worker memory limit: MemoryMax=$(meta_value memory_max))"
    fi
  fi
  ...
  case "$TASK_BACKEND:$AGENT_STATE" in
    ...
    tmux:missing|herdr:missing)
      emit unknown none "backend target gone: $BACKEND_TARGET$GONE_SUFFIX"
      ;;
    tmux:dead|herdr:dead)
      emit unknown none "backend target gone: $BACKEND_TARGET (agent gone, pane shell remains)$GONE_SUFFIX"
      ;;
    tmux:*|herdr:*)

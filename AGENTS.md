...
The per-file inventory of tracked files, `config/`, `data/`, and `state/` entries lives in `docs/configuration.md` "File inventory".
Never edit or delete a `state/` record that inventory marks "never touch"; only its owning script writes it.
Ignored local directories are `.env`, `config/`, `data/`, `state/`, `projects/`, and `.no-mistakes/`.

...
state/               runtime records and signals; gitignored
  ...
  .afk               durable away/quiet-mode flag; present = sub-supervisor may inject escalations, first line `away` (default, set by /afk, cleared on user return) or `quiet` (set by /quiet, cleared only on explicit /quiet off) per the single owner fm_afk_mode() in bin/fm-wake-lib.sh
...
Use `gh-axi` for GitHub and `chrome-devtools-axi` for browser work; consult current help rather than memorizing flags.
...
If `state/.afk` is present, load `/afk` in away mode or `/quiet` in quiet mode (`bin/fm-wake-lib.sh`'s `fm_afk_mode`) and let its daemon own supervision rather than arming another cycle.
...
Invoke the `/afk` skill when the captain says `/afk`, says they are going afk, `state/.afk` exists in away mode, an incoming message starts with `FM_INJECT_MARK`, or any `state/.subsuper-*` marker is involved.
...
Use plain chat for a yes-or-no decision and a plain static HTML page served at an http URL only when several options or a structured report benefit from a visual surface.

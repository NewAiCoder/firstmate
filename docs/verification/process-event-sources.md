...
| reconcile failure surfaced once per episode | a state root that regresses to group- or world-writable and fails that same private-directory check is reported by the watcher as a single actionable `check: process-event reconcile failed:` wake per failure episode - never swallowed, never repeated every cycle - and the episode marker clears once reconcile succeeds again; a reconcile that exits non-zero without an error message (an unconfirmed runner launch, already announced on its own) raises no such wake (`tests/fm-watch-triage.test.sh`) |
| symlinked state-root leaf diagnosed by resolved mode | a state root that is itself a symlink to a directory regressed to group-writable is diagnosed as `bad-mode`, not `missing`: only the state-root callers ask the diagnostic to resolve the leaf to its physical target before checking mode, while the shared private-directory validity check still rejects any symlinked leaf so evidence writes can never be redirected (`tests/fm-procevent.test.sh`, `tests/fm-extension-binding.test.sh`) |

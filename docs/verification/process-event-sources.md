...
| rebind-all reaches a watch whose poller is already running | `run` reloads the trust binding from disk immediately before claiming the fire instead of trusting the hash cached at `spec_load` time when its poll loop started, so a watch already polling when a self-update's `rebind-all` runs still fires against the rebound bytes rather than being wrongly rejected as stale (`tests/fm-procevent-when.test.sh`) |
| the fire-time reload never observes rebind_one's publish mid-rename | the fire-time reload is bracketed by the same per-sid lock `rebind_one` holds while it renames the new spec then the new trust into place, so a poller reaching its fire exactly during that two-step publish waits past the torn window instead of reading a spec already rebound to v2 next to a trust record still bound to v1 (`tests/fm-procevent-when.test.sh`) |

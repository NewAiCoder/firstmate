...
It is deliberately narrow: only a `turn-ended` file (a status append always wakes), only while that task's own marker is in an in-flight phase, only within `FM_IDLE_COMPACT_SAVE_TIMEOUT_SECS` of the send that induced it, at most one turn per send, and only when the watcher's `.seen-*` marker shows no earlier turn-end still unannounced.

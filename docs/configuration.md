...
The cap binds round count, not only the pipeline's own `auto_fix.review` loop. With `auto_fix.review` at 0 the pipeline does not loop reviews on its own, and a worker drives convergence by calling `no-mistakes axi respond --action fix` itself after each review; `bin/fm-dod-lib.sh` tells the worker that every such worker-driven fix response is a round exactly like a pipeline-counted one, tallied on the same run against the same round-3 soft cap and severity escape. Field evidence (2026-09-11) is one task that burned 11+7 cold review rounds and roughly 117M tokens under `auto_fix.review=0` because nothing bounded worker-driven rounds; the cap now applies regardless of which side - pipeline or worker - is doing the counting.
...
`bin/fm-brief.sh` warns at scaffold time, when splitting is still cheap, whenever a `ship`-kind brief carries more than 5 acceptance items (`FM_BRIEF_ACCEPTANCE_ITEMS`); it never refuses, because firstmate may have a good reason. Item 8 of the ship brief's Rules also tells the worker to run `bin/fm-diff-size-check.sh .`, timed per the task's delivery mode: before its first `no-mistakes axi run` for mode=no-mistakes (the Definition-of-done block says how), before pushing and opening the PR for mode=direct-PR, and before reporting the branch ready for mode=local-only. The check resolves its base branch the same way as the rest of the fleet (`fm_default_branch`: origin/HEAD, then local main, then local master) rather than guessing, so it always exits 0 and prints one of `ok`, `over-target`, or `over-cap` against a real base - or a "cannot measure" line when no default branch resolves at all, instead of silently diffing HEAD against itself. `over-cap` (over 800 changed lines) is a stop-and-ask-firstmate point, not a refusal or a gate — the worker notes the size and lets firstmate decide whether to split.

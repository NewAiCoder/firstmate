...
4. `phase=done`: compared on every sweep against the current status line and pane-tail signature; either changing (a new status append or new pane activity) clears the marker so the next idle episode is evaluated fresh, exactly the reset condition the requirement calls for.

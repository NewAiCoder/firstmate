...
`FM_IDLE_COMPACT_RING_BACKSTOP_SECS` (default 600) then guards the residue: an unchanged `phase=done` marker older than that, over a status line that still declares the pause, with no ring record anywhere in the task's inbox (pending or acknowledged), gets one re-send and one line in `state/.idle-compact.log`.
...
Every message is delivered through `bin/fm-send.sh` exactly as any other steer - the notes save and the `/compact` command on its typed plane, with type-once-verified-Enter submission, per-harness settle timing and delivery confirmation unchanged; the ring on its durable inbox plane - and idle-compact never calls a backend primitive or raw tmux command directly.

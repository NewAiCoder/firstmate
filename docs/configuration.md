...
This inventory is the single owner of the per-file entries under the tracked code root, `config/`, `data/`, and `state/`.
`AGENTS.md` keeps only the directory roles and the safety rules that apply to them.
Entries marked "never touch" are internal coordination records that only their named owner writes.
Producer script headers still own exact child fields and mutation contracts.
...
.env                 optional Relay pairing token (presence-gates `AGENTS.md` section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and typed dispatch resolution key TYPESAFE_API_KEY (presence-gates bin/fm-dispatch-resolve.sh; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  projects.md        thin fleet navigation registry recording each project's standing delivery posture and optional ship-branch prefix; firstmate-private, parsed by fm-project-mode.sh (`AGENTS.md` section 6)

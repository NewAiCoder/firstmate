...
When a worker is killed by its own `MemoryMax`, `bin/fm-watch.sh`'s general stale-classification path (`surface_nonterminal_stale`, first sighting of a stale window) checks the recorded scope's `systemctl --user show -p Result` as soon as the endpoint is independently confirmed dead, and - only when `Result` is exactly `oom-kill`, and only once per task (`state/<id>.oom-reported`, removed by teardown) - appends `failed: killed by the per-worker memory limit (MemoryMax=...)` to the task's own status log, so firstmate escalates on the actual cause instead of guessing at a generic wedge.
...
An optional SECOND non-empty, non-comment content line sets the declared-state settle seconds described below (whole positive integer seconds); absent, empty, or malformed falls back to the documented 60-second default, the same typo-must-never-fail contract as the first line.
...
Because that worker is waiting on firstmate, not settling out its own pane activity, the declared-state path's `settling` wait is short: `FM_IDLE_COMPACT_SETTLE_SECS` when set (mainly for tests), otherwise config/idle-compact's optional second content line in seconds, defaulting to 60 - not the full `FM_IDLE_COMPACT_INTERVAL` sweep the ordinary path waits (see [`NewAiCoder/firstmate#34`](https://github.com/NewAiCoder/firstmate/issues/34): reacting to the declared line already happens within one sweep, but the pre-fix settle-then-ring wait added up to another full interval on top, idling a lane for minutes after a compaction that itself takes seconds).
...
3. `phase=settling`: wait for the compaction summary to finish rendering, then record `phase=done` with the status line and pane-tail signature at that moment - captured after the render, so the compaction's own output is baked into the baseline and never reads as new worker activity (which would otherwise re-trigger a fresh save+compact episode every couple of sweeps).
   The wait is `FM_IDLE_COMPACT_SETTLE_SECS` when set; otherwise a declared episode uses the 60-second-default declared-state settle seconds above, while an ordinary episode keeps waiting one full sweep interval (`FM_IDLE_COMPACT_INTERVAL`, default 300s).
...
The escape is not a hedge. Measured over 2026-08-29 to 09-05 across firstmate and two private projects, rounds 4 and up produced class-a and class-b findings (data correctness, security, data loss, real functional bugs) at 6.2% against 4.9% for rounds 1 to 3, and 33.3% in one of them.
Four of the private Django project's round-4-plus fixes are merged to production, including a management command with no dev-settings guard that overwrote the live check-in row and a teardown command that cascade-deleted linked financial records.
...
`--external-sources` (which this script always passes) makes ShellCheck recursively analyze every sourced file, so a heavily-cross-sourcing hub script legitimately peaks far higher than its own size suggests: measured directly (`systemd-run` + `/usr/bin/time -v`, 2026-09-06) against every file a flat 1 GiB ceiling had false-failed, 23 real files peaked between 1.0 and 3.9 GiB and finished in under a minute each, and 4 more never stabilized even under a 4 GiB / 180 s probe.
`FM_LINT_FILE_MEM_KB`'s default is therefore not one flat number: it targets `6291456` KiB (6 GiB, clearing the largest measured real peak with headroom, matching the existing `worker_memory_max` convention above) but is capped by 70% of this host's own `MemTotal` divided by the concurrent shard count (`FM_LINT_JOBS`, default 2) - the same 70% headroom convention `slice_memory_max_pct` uses - so the ceiling can never authorize more concurrent memory across shards than a small CI runner actually has, which is exactly how a false-positive on one file turned into collateral damage on someone else's build in the first place. An explicit `FM_LINT_FILE_MEM_KB` always overrides this computed default untouched. `fm_lint_default_file_mem_kb` in `bin/fm-lint.sh` owns the exact formula.
...
Pipeline seats get the same surface through the operator's personal harness agent-args template.
...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.46.0 or newer, compatible gh-axi, chrome-devtools-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
...
`tasks-axi` and `quota-axi` are required bootstrap tools in every profile, the same class as `gh-axi`.
...
`lavish-axi` is not a bootstrap tool: bootstrap neither detects nor floor-checks it, so a home without it stays silent, and the lavish-consuming adapters below report their own missing binary only if invoked.
...
FM_CLAUDE_PARK_SECONDS=27000   # seconds one Claude Stop-owned watcher park runs before closing itself with a rewake; must stay below the hook's 28800s timeout, and other values fall back to 27000

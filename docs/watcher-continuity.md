...
The hook fires on every Stop, and an eligible primary with supervision need admits one home-scoped owner that runs `bin/fm-watch-arm.sh` as a tracked child it waits on inside the hook-owned process tree.
...
Claude kills the hook at its registered 28800-second timeout without delivering any rewake, and a quiet fleet absorbs every no-change heartbeat, so the owner closes its own park before then and wakes the session through that same exit 2; the hook header owns this park boundary, and `fm_claude_park_seconds` in `bin/fm-wake-lib.sh` owns its value.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, exit-2 translation, the park boundary closing a quiet park, and supersession of a claim older than that boundary.

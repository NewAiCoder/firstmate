...
Claude's `bin/fm-claude-stop-autoarm.sh` hook fires on every Stop and, when the home is eligible and still needs supervision, claims one home-scoped cycle, runs the arm wrapper as a tracked child it waits on, and translates actionable closes - including its own park boundary closing a quiet cycle before the hook's registered timeout, per [`watcher-continuity.md`](watcher-continuity.md) - into exit-2 rewakes.
...
While the record exists, GitHub auto-merge is refused, and GitLab auto-merge flags or scheduled state are refused while an immediate merge is forced with a final `--auto-merge=false`.
A GitHub base whose rules cannot prove the absence of a merge queue is also refused before submission unless the task is in the record's merge-grant list, because a named grant is the captain's own decision for that merge while a standing yolo posture is not; `refuse_github_queue_while_away` in `bin/fm-pr-merge.sh` owns that rule.
This is deliberately confused-agent-grade, as `bin/fm-lease-lib.sh` defines that grade, rather than fully atomic.
A GitHub queue-rule or PR-base change after the queue-free preflight, or a queue on a granted task's base, can still enqueue a merge that lands after its away grant lapses, and killing the lock-owning shell while its forge child survives lets stale-owner recovery admit archive or replacement before that child completes.

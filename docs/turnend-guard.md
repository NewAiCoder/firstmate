...
`fm_autoarm_claim_open`'s own comment in `bin/fm-wake-lib.sh` owns the full open/stuck/expired predicate, including the `fm_claude_park_seconds` expiry that closes a claim before the hook's own 28800-second timeout above would; anything that predicate rejects lets the next Stop-owned firing take the next generation and arm, and taking a newer generation is the reclaim - a steady-state predecessor is never signalled or revoked.

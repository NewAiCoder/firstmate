...
This derivation is local to the hook-owned auto-arm chain and to the watcher's own default; other direct `FM_GUARD_GRACE` readers (`bin/fm-guard.sh`, `bin/fm-turnend-guard.sh` and its harness-specific wrappers, `bin/fm-wake-lib.sh`) still fall back to the bare 300-second default unless `FM_GUARD_GRACE` is set explicitly in the environment.

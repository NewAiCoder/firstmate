...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\"}' --setting-sources project,local --strict-mcp-config --mcp-config '/tmp/fm-$id/mcp.json' \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/launch-brief.md')\""
...
test_claude_threads_model_and_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\"}' --setting-sources project,local --strict-mcp-config --mcp-config '/tmp/fm-$id/mcp.json' --model 'sonnet' --effort 'high'" \

...
A merge under away authority must be synchronous; `fm-pr-merge.sh` refuses auto-merge for every task while the record exists.
It also refuses a GitHub merge whose queue state cannot prove an immediate merge, unless the task is in the record's merge-grant list, because a named grant is the captain's own decision for that merge while yolo alone is not.
